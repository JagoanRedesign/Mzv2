...
@app.on_message(filters.command("ytvideo") & filters.private)


async def yt_vid(client: Client, message: Message):
    input_st = message.text
    input_str = input_st.split(" ", 1)[1]
    Man = await message.reply_text(message, "`Processing...`")
    if not input_str:
        await Man.edit_text(
...
@app.on_message(filters.command("ytsong") & filters.private)
